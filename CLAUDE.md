...
Work proceeds in phases — see `ROADMAP.md` for the current phase and what's deliberately deferred. Phases 1–7 are complete; phase 8's core optimizations (scope locals, int fast path) are in — its release items (disk cache, context flattening, 0.1.0) remain.
