...
- `src/dtc/compiler.py` — the compiler. Contract: `compile_template(template) -> callable(Context) -> str | None`. Returning `None` means "not compilable yet"; the fallback makes the engine correct by construction while codegen coverage grows tag-by-tag. Phases 1–4 are implemented: text, variables (filters call the registered functions directly, with `is_safe`/`needs_autoescape`/`expects_localtime` specialization decided at compile time — custom filters compile natively), `{% if %}`, `{% for %}`, `{% with %}`, `{% autoescape %}`, `{% comment %}`, `{% verbatim %}`, `{% load %}`, and inheritance (`{% block %}`/`{% extends %}`/`{% include %}`). Scoping is done with *real* `context.push()/pop()/__setitem__` calls so slow-path replays and filter args always see live context; the `forloop` dict is elided when the analysis pass proves it unreferenced (a `{% block %}`/`{% include %}` inside a loop disables elision — overridden/included content can reference `forloop` invisibly).
